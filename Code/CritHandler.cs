...
public class StreamType
{
    ...
    public class CritSfxHandler : ModPlayer
    {
        ...
        {
            if (!Directory.Exists(Main.SavePath + Path.DirectorySeparatorChar + "Crit Sounds" + Path.DirectorySeparatorChar + "Custom"))
            {
        ...
        public override void OnHitNPCWithProj(Projectile proj, NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                var rangedVolume = ModContent.GetInstance<CritSoundsConfig>().Mod_TypeRanged_Volume;

                if (_rangedCritsFiles.Count == 0)
                {
                    SoundEngine.PlaySound(CritContainers.TypeRangedCritsSound, target.position);
                }
            ...
            {
                var throwingVolume = ModContent.GetInstance<CritSoundsConfig>().Mod_TypeThrowing_Volume;

                if (_throwingCritsFiles.Count == 0)
                {
                    SoundEngine.PlaySound(CritContainers.TypeThrowingCritsSound, target.position);
                }
            ...
            if (proj.DamageType == DamageClass.Magic || proj.DamageType == DamageClass.MagicSummonHybrid)
            {
                ...
                {
                    SoundEngine.PlaySound(CritContainers.TypeMagicCritsSound, target.position);
                }
            ...
            {
                var meleeVolume = ModContent.GetInstance<CritSoundsConfig>().Mod_TypeMelee_Volume;

                if (_meleeCritsFiles.Count == 0)
                {
                    SoundEngine.PlaySound(CritContainers.TypeMeleeCritsSound, target.position);
                }
            ...
            {
                var summonVolume = ModContent.GetInstance<CritSoundsConfig>().Mod_TypeSummon_Volume;

                if (_summonCritsFiles.Count == 0)
                {
                    SoundEngine.PlaySound(CritContainers.TypeSummonCritsSound, target.position);
                }
            ...
            if (proj.DamageType != DamageClass.Generic) return;
            {
                var genericVolume = ModContent.GetInstance<CritSoundsConfig>().Mod_TypeGeneric_Volume;

                if (_genericCritsFiles.Count == 0)
                {
                    SoundEngine.PlaySound(CritContainers.TypeGenericCritsSound, target.position);
                }

                if (_genericCritsFiles.Count == 0) return;
